...
public class CSharpTokenMakerTest extends AbstractTokenMakerTest {
	...
	public void testCharLiterals() {
		...
		for (String code : chars) {
			...
			TokenMaker tm = createTokenMaker();
			Token token = tm.getTokenList(segment, TokenTypes.NULL, 0);
			Assert.assertEquals("Not a char literal: " + code, TokenTypes.LITERAL_CHAR, token.getType());
		}
	...
	public void testDataTypes() {
		...
		for (int i = 0; i < dataTypes.length; i++) {
			Assert.assertEquals(dataTypes[i], token.getLexeme());
			Assert.assertEquals("Not a data type: " + dataTypes[i], TokenTypes.DATA_TYPE, token.getType());
	...
	public void testFloatingPointLiterals() {
		...
		for (int i = 0; i < literals.length; i++) {
			...
			Assert.assertEquals("Not a float: " + literals[i], TokenTypes.LITERAL_NUMBER_FLOAT, token.getType());
	...
	public void testHexLiterals() {
		...
		for (int i = 0; i < literals.length; i++) {
			Assert.assertEquals(literals[i], token.getLexeme());
			Assert.assertEquals("Not a hex literal: " + literals[i], TokenTypes.LITERAL_NUMBER_HEXADECIMAL, token.getType());
			if (i < literals.length - 1) {
				token = token.getNextToken();
				Assert.assertTrue("Not a whitespace token: " + token, token.isWhitespace());
				Assert.assertTrue(token.is(TokenTypes.WHITESPACE, " "));
	...
	public void testKeywords() {
		...
		for (int i = 0; i < keywords.length; i++) {
			Assert.assertEquals(keywords[i], token.getLexeme());
			Assert.assertEquals("Not a keyword: " + keywords[i], TokenTypes.RESERVED_WORD, token.getType());
			if (i < keywords.length - 1) {
				token = token.getNextToken();
				Assert.assertTrue("Not a whitespace token: " + token, token.isWhitespace());
				Assert.assertTrue(token.is(TokenTypes.WHITESPACE, " "));
